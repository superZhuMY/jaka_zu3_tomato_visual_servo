...
from ultralytics.utils.general import xyxy2xywh, xywh2xyxy, clean_str
from ultralytics.utils.torch_utils import torch_distributed_zero_first
from ultralytics.utils.augmentations import *
...
def create_road_seg_dataloader(path, clsnum, imgsz, batch_size, stride, single_cls=False, hyp=None, augment=False,
                               cache=False,
                               pad=0.0,
...
class LoadFaceImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.batch = bi  # batch index of image   图像批量索引
        self.n = n
        self.indices = range(n)
        # self.counter_per_cls = [0]
        # for label in labels:
        #     labelnum = [label[it, 0] for it in range(len(label))]
        #     for it in labelnum:
        #         self.counter_per_cls[int(it)] += 1
            ...
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    if len(l):
                        ...
                        assert l.shape[1] ==7, 'labels require 15 columns each'
    ...
    def __getitem__(self, index):
        ...
        if nL:
            ...
            labels[:, [5]] /= img.shape[1]  # normalized landmark x 0-1
            labels[:, [5]] = np.where(labels[:, [5]] < 0, -1, labels[:, [5]])
            labels[:, [6]] /= img.shape[0]  # normalized landmark y 0-1
            labels[:, [6]] = np.where(labels[:, [6]] < 0, -1, labels[:, [6]])
...
def load_seg_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        # Load image
        img, label, _, (h, w) = load_segimagelabel(self, index)
...
class LoadSegImagesAndLabels(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if random.random() < mosaic and not self.val:
            ...
            # Load mosaic
            img, labels = load_seg_mosaic(self, index)
            # for l in range(len(labels)):
            #     # print(labels)
            #     cv2.putText(img, str(labels[l][0]), (int(labels[l][1]),int(labels[l][2])), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255,0), 2)
            #
            #     cv2.rectangle(img,(int(labels[l][1]),int(labels[l][2])),(int(labels[l][3]),int(labels[l][4])), (0, 255, 0), 2)
            # cv2.imwrite("masic/masic_{}.jpg".format(index),img)
            ...
            # mask[:, :, 2] = labels * 20
            #
            # vis_img = 0.7*img + 0.3+mask
            #
            # cv2.imwrite("segmasic/segmasic_{}.png".format(index),vis_img)
            # cv2.imwrite("segmasic/segmasic_{}_mask.png".format(index), labels)
...
def random_perspective(img, targets=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0, border=(0, 0)):
    ...
    if n:
        ...
        xy[:, :2] = targets[:, [1, 2, 3, 4, 1, 4, 3, 2, 5, 6]].reshape(n * 5, 2)
        xy = xy @ M.T  # transform
        if perspective:
        ...
        landmarks[:, [0]] = np.where(landmarks[:, [0]] > width, -1, landmarks[:, [0]])
        landmarks[:, [1]] = np.where(landmarks[:, [1]] > height, -1,landmarks[:, [1]])
