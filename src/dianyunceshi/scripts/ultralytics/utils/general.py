...
from ultralytics.utils.google_utils import gsutil_getsize
...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False,EIoU=False,SIoU=False, eps=1e-9):
...
def box_iou(box1, box2):
    ...
    aa=box1[:, None, 2:]
    bb= box2[:, 2:]
    cc=torch.min(box1[:, None, 2:], box2[:, 2:])

    inter1 = (torch.min(box1[:, None, 2:], box2[:, 2:]) -
             torch.max(box1[:, None, :2], box2[:, :2])).clamp(0)
    # print(aa, bb, cc,inter1)
...
def non_max_suppression_face(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, labels=()):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Compute conf
        x[:, 7:] *= x[:, 4:5]  # conf = obj_conf * cls_conf
        ...
        if multi_label:
            ...
            x = torch.cat((box[i], x[i, j + 7, None], x[i, 5:7] ,j[:, None].float()), 1)
        else:  # best class only
            conf, j = x[:, 7:].max(1, keepdim=True)
            x = torch.cat((box, conf, x[:, 5:7], j.float()), 1)[conf.view(-1) > conf_thres]
