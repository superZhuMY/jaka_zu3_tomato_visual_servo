...
from ultralytics.utils.general import LOGGER
...
def validate(testloader, model, numcls, criterion):
    ...
    # testloader.sampler.set_epoch(8)

    s = ('%11s' * 3) % ('Images', 'Ave Loss', 'mIOU')

    with torch.no_grad():
        for idx, batch in enumerate(tqdm(testloader,desc=s, bar_format='{l_bar}{bar:10}{r_bar}{bar:-10b}')):
